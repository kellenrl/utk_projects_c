 ...
 *
 * This is the main src file for lab2 part 1
 */
...
int main (int argc, char *argv[])
{
	...
	//read the entire file into a buffer and close the file
	fseek(fin, 0, SEEK_END);
	long long fsize = ftell(fin);
	fseek(fin, 0, SEEK_SET);

	fbuf = malloc(fsize + 1);
	nrd = fread(fbuf, 1, fsize, fin);
	fbuf[fsize] = '\0';
	...
	//create jrb tree for hosts, allocates memory to store names
	hosts = make_jrb();
	arrCnt = 0;
